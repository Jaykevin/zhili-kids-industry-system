...
function loadTableData(tableBodyId) {
    ...
    // 初始化分页
    PaginationManager.init(type);
    
    // 加载当前页数据
    PaginationManager.loadPageData(type);
}
...
const SupplyDemandManager = {
    ...
    loadDemandsFromAPI: async function() {
        ...
        try {
            ...
            if (result.code === 200 && result.data) {
                this.apiData.demands = result.data.map(item => this.mapDemandFromAPI(item));
                this.pagination.demand.totalRecords = result.data.length;
                this.pagination.demand.totalPages = Math.ceil(result.data.length / this.pagination.demand.pageSize);
    ...
    _showConnectionChatWindow: function(connectionId, otherUserId, otherCompanyName, myUserId) {
        ...
        // 初始化 WebSocket 聊天订阅
        this._initConnectionChatWebSocket(connectionId, otherUserId);
    },
    ...
    _loadConnectionChatHistory: async function(connectionId, otherUserId) {
        ...
        try {
            var response = await fetch(API_BASE_URL + '/chat/messages?otherUserId=' + encodeURIComponent(otherUserId) + '&connectionId=' + connectionId, {
    ...
    _sendConnectionMessage: async function(connectionId, otherUserId) {
        ...
        try {
            var response = await fetch(API_BASE_URL + '/chat/send', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + token
    ...
    _markConnectionChatRead: async function(connectionId) {
        var token = localStorage.getItem('token');
        try {
            await fetch(API_BASE_URL + '/chat/mark-connection-read', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + token
    ...
    /** 初始化对接聊天的 WebSocket 订阅 */
    _initConnectionChatWebSocket: function(connectionId, otherUserId) {
        var self = this;

        // 监听个人消息频道
        if (this.wsClient && this.wsConnected) {
            var subscription = this.wsClient.subscribe('/topic/chat/user/' + otherUserId, function(message) {
                var msg = JSON.parse(message.body);
                // 只处理属于这个对接的消息
                if (msg.connectionId === connectionId) {
                    self._appendIncomingConnectionMessage(msg, connectionId);
                }
            });
    ...
    /** 追加收到的对接聊天消息 */
    _appendIncomingConnectionMessage: function(msg, connectionId) {
        var msgContainer = document.getElementById('connectionChatMessages');
        if (!msgContainer) return;
    ...
    showMyPublishedItems: async function() {
        ...
        try {
            ...
            var body = document.getElementById('msgListBody');
            if (body) {
                body.innerHTML = '<div class="empty-state" style="color:#e74c3c"><i class="bi bi-exclamation-triangle"></i> 加载失败，请检查网络连接</div>';
            }
        }
    },
    ...
    _switchSentTab: async function(btn) {
        ...
        try {
            ...
            result.data.forEach(function(conn) {
                ...
                if (conn.status === 'negotiating') {
                    html += '<div class="conn-actions">';
                    html += '<button class="btn-sm btn-primary" onclick="SupplyDemandManager._openConnectionChat(' + conn.id + ', \'' + (conn.demandUserId || conn.supplyUserId || '') + '\', \'' + escapeHtml(conn.demandCompanyName || conn.supplyCompanyName || '对方') + '\')"><i class="bi bi-chat-dots"></i> 发起聊天</button>';
            ...
            var sentListEl = document.getElementById('sentConnectionsList');
            if (sentListEl) {
                sentListEl.innerHTML = '<div class="empty-state" style="color:#e74c3c"><i class="bi bi-exclamation-triangle"></i> 加载失败，请检查网络连接</div>';
            }
        }
    },
